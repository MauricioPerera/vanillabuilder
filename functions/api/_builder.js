...
const TEMPLATES = {
  ...
  features: (o = {}) => {
    ...
    const cols = items.map(i => `<div style="flex:1;min-width:250px;padding:24px;background:${o.cardBg||'#f8f9fa'};border-radius:8px;text-align:center;">
      ...
      <p style="color:${o.textColor||'#666'};font-size:14px;line-height:1.6;">${i.description}</p></div>`).join('');
    return `<section style="padding:${o.padding||'60px 20px'};max-width:${o.maxWidth||'960px'};margin:0 auto;">
      ...
      <div style="display:flex;gap:24px;flex-wrap:wrap;">${cols}</div></section>`;
  },
  ...
  testimonials: (o = {}) => {
    ...
    const cards = items.map(i => `<div style="flex:1;min-width:300px;padding:32px;background:#f8f9fa;border-radius:12px;">
      ...
      <div><strong style="color:#333;">${i.author}</strong><br/><span style="font-size:13px;color:#888;">${i.role||''}</span></div></div>`).join('');
    return `<section style="padding:${o.padding||'60px 20px'};max-width:960px;margin:0 auto;">
      ${o.heading?`<h2 style="text-align:center;font-size:32px;margin-bottom:40px;color:#333;">${o.heading}</h2>`:''}
      <div style="display:flex;gap:24px;flex-wrap:wrap;">${cards}</div></section>`;
  },
  ...
  pricing: (o = {}) => {
    ...
    const cards = plans.map(p => `<div style="flex:1;min-width:260px;max-width:300px;padding:32px;background:white;border-radius:12px;text-align:center;border:${p.popular?'2px solid #7c3aed':'1px solid #e2e8f0'};position:relative;">
      ...
      <a href="${p.buttonUrl||'#'}" style="display:block;padding:12px;background:#7c3aed;color:white;border-radius:6px;text-decoration:none;font-weight:600;">${p.buttonText||'Choose Plan'}</a></div>`).join('');
    return `<section style="padding:${o.padding||'60px 20px'};background:${o.background||'#f8f9fa'};">
      ${o.heading?`<h2 style="text-align:center;font-size:32px;margin-bottom:40px;color:#333;">${o.heading}</h2>`:''}
      <div style="display:flex;gap:24px;justify-content:center;flex-wrap:wrap;max-width:960px;margin:0 auto;">${cards}</div></section>`;
  },
  ...
  footer: (o = {}) => {
    ...
    const colsHtml = cols.map(c => `<div style="flex:1;min-width:150px;">
      ...
      ${c.links?`<div style="display:flex;flex-direction:column;gap:8px;">${c.links.map(l=>`<a href="${l.url||'#'}" style="color:#9ca3af;text-decoration:none;font-size:14px;">${l.text}</a>`).join('')}</div>`:''}</div>`).join('');
  ...
  faq: (o = {}) => {
    ...
    const html = items.map(i => `<div style="border-bottom:1px solid #e2e8f0;padding:16px 0;">
      <h4 style="font-size:16px;color:#333;margin-bottom:8px;">${i.question}</h4>
      <p style="font-size:14px;color:#666;line-height:1.6;">${i.answer}</p></div>`).join('');
    return `<section style="padding:${o.padding||'60px 20px'};max-width:700px;margin:0 auto;">
      ${o.heading?`<h2 style="text-align:center;font-size:32px;margin-bottom:40px;color:#333;">${o.heading}</h2>`:''}${html}</section>`;
  },
  ...
  stats: (o = {}) => {
    ...
    const html = items.map(i => `<div style="flex:1;min-width:120px;text-align:center;">
      <div style="font-size:36px;font-weight:800;color:${o.valueColor||'#7c3aed'};margin-bottom:4px;">${i.value}</div>
      <div style="font-size:14px;color:${o.labelColor||'#666'};">${i.label}</div></div>`).join('');
    return `<section style="padding:${o.padding||'60px 20px'};background:${o.background||'#f8f9fa'};">
      <div style="display:flex;gap:32px;flex-wrap:wrap;justify-content:center;max-width:800px;margin:0 auto;">${html}</div></section>`;
  },
};

// ── PageBuilder (stateful, no DOM) ──
...
export class PageBuilder {
  ...
  getHTML() {
    return { ok: true, data: this.sections.join('\n') };
  }

  getCSS() {
    return { ok: true, data: this.cssRules.join('\n') };
  }
...
${body}
  ...
  getPageInfo() {
    return { ok: true, data: { sectionCount: this.sections.length, cssRuleCount: this.cssRules.length } };
  }

  getAvailableSections() {
    return { ok: true, data: Object.keys(TEMPLATES).map(id => ({
      id,
      description: { hero:'Hero banner', features:'Feature cards grid', cta:'Call to action', testimonials:'Quote cards', pricing:'Pricing plans', footer:'Page footer', contact:'Contact form', faq:'FAQ list', navbar:'Navigation bar', stats:'Stats counters' }[id]
    })) };
  }
